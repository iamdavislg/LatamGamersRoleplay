...
// PCU
const pcuBtn = document.querySelector('.nav-link[href="#pcu"]');
const popupPCU = document.getElementById('popupPCU');
const cerrarPCU = document.getElementById('cerrarPCU');

if (pcuBtn && popupPCU && cerrarPCU) {
  pcuBtn.addEventListener('click', e => {
    e.preventDefault();
    popupPCU.classList.remove('hidden');
  });
  cerrarPCU.addEventListener('click', () => {
    popupPCU.classList.add('hidden');
  });
}
